   ...
   (test-case
    ...
    (let ([xlsx (new xlsx%)])
      ...
      (let* ([style_hash (send xlsx get-style-hash)]
        ...
        (check-equal? (hash-ref style (quote fgColor) "blue"))
      ...
      (send xlsx set-data-sheet-cell-style! #:sheet_name "测试1" #:cell_range "D1-D4" #:style '( ('fgColor . "red") ))
      ...
      (let* ([sheet (sheet-content (send xlsx get-sheet-by-name "测试1"))]
       ...
       (let* ([style_index (hash-ref range_to_index_hash "D1-D4")]
              [style (list-ref style_list style_index)])
